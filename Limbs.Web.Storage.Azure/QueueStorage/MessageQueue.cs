...
namespace Limbs.Web.Storage.Azure.QueueStorage
{
	...
	public class MessageQueue<TMessage> where TMessage : class
	{
		...
		{
			var queueRef = _queueClient.GetQueueReference(_queueName);
			var messages = queueRef.GetMessages(messagesCount < MaxMessageBlockAllowedByAzure ? messagesCount : MaxMessageBlockAllowedByAzure);
		...
		{
			var queueRef = _queueClient.GetQueueReference(_queueName);
			var messages = queueRef.GetMessages(messagesCount < MaxMessageBlockAllowedByAzure ? messagesCount:MaxMessageBlockAllowedByAzure, timeout);
			return messages.Select(ConvertToQueueMessage);
		}
		...
		{
			var queueRef = _queueClient.GetQueueReference(_queueName);
			var messages = queueRef.PeekMessages(messagesCount < MaxMessageBlockAllowedByAzure ? messagesCount : MaxMessageBlockAllowedByAzure);
			return messages.Select(ConvertToQueueMessage);
		}
